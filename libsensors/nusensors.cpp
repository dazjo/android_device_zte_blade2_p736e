...
sensors_poll_context_t::sensors_poll_context_t()
{
    if(fopen(TAOS_DEVICE_NAME,"rw")) {
        ...
        mSensors[prox] = new Blade2Proximity(const_cast<char *>(TAOS_DEVICE_NAME));
        mPollFds[prox].fd = mSensors[prox]->getFd();
        mPollFds[prox].events = POLLIN;
        mPollFds[prox].revents = 0;

        mSensors[light] = new Blade2Light(const_cast<char *>(TAOS_DEVICE_NAME));
        mPollFds[light].fd = mSensors[light]->getFd();
        mPollFds[light].events = POLLIN;
        mPollFds[light].revents = 0;
    } else {
        ALOGD("Using ISL prox/light sensor");
        mSensors[prox] = new Blade2Proximity(const_cast<char *>(ISL_DEVICE_NAME));
        mPollFds[prox].fd = mSensors[prox]->getFd();
        mPollFds[prox].events = POLLIN;
        mPollFds[prox].revents = 0;

        mSensors[light] = new Blade2Light(const_cast<char *>(ISL_DEVICE_NAME));
        mPollFds[light].fd = mSensors[light]->getFd();
        mPollFds[light].events = POLLIN;
        mPollFds[light].revents = 0;
    }
